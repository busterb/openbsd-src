...
/*
 * Shared kprobe provider for amd64 and i386.
 *
 * Both architectures instrument function entry/return with a single-byte
 * int3 breakpoint and emulate the overwritten instruction in the trap
 * handler, so the probe bookkeeping, allocation, and breakpoint dispatch
 * are identical.  Only prologue/epilogue byte-pattern recognition and the
 * trapframe register layout differ; those live in the per-arch
 * dt_prov_kprobe.c files, which provide:
 *
 *   int	 db_prologue_validate(Elf_Sym *);
 *   void db_prologue_patch(vaddr_t, int);
 *   int	 db_epilogue_validate(Elf_Sym *, int);
 *   void db_epilogue_patch(vaddr_t, int);
 *   void dt_prov_kprobe_getargs(struct trapframe *, register_t *);
 *   register_t dt_prov_kprobe_getretval(struct trapframe *);
 */
...
dt_prov_bkpt_hook(struct trapframe *tf)
{
	...
	SLIST_FOREACH(dtp, &dtpf_entry[INSTTOIDX(addr)], dtp_knext) {
		...
		SMR_SLIST_FOREACH(dp, &dtp->dtp_pcbs, dp_pnext) {
			...
			if (ISSET(dp->dp_evtflags, DTEVT_FUNCARGS))
				dt_prov_kprobe_getargs(tf, dtev->dtev_args);
	...
	SLIST_FOREACH(dtp, &dtpf_return[INSTTOIDX(addr)], dtp_knext) {
		...
		SMR_SLIST_FOREACH(dp, &dtp->dtp_pcbs, dp_pnext) {
			...
			dtev->dtev_retval[0] = dt_prov_kprobe_getretval(tf);
